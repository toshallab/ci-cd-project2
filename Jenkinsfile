  ...
  stages {
	...
	post {
		success {
			script {
			if (env.BRANCH_NAME == 'master'){
				withCredentials([usernamePassword(credentialsId: '103348281', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
				sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/toshallab/ci-cd-project2.git HEAD:master')
					}
				} else if (env.BRANCH_NAME == 'dev'){
